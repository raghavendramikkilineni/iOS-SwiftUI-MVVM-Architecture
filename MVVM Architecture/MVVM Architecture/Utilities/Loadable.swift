...
enum Loadable<T> {
    ...
    var value: T? {
        ...
        case let .isLoading(last: last, _): return last
        default: return nil
        }
    }
    
    var bindingUnwrappedValue: T {
        get {
            return value!
        }
        set(newValue) {
    ...
    mutating func cancelLoading() {
        ...
        case let .isLoading(last, cancelBag):
            ...
            if let last = last {
                ...
                let error = NSError (domain: NSCocoaErrorDomain, code: NSUserCancelledError, userInfo: [NSLocalizedDescriptionKey: NSLocalizedString("Error Occured.", comment: "")])
                self = .failed(error)
            }
        default:
            break
...
protocol LoadableView {
    ...
    var content: AnyView { mutating get }
    
    func notRequestedView() -> AnyView
    func loadingView(_ previouslyLoaded: Result?) -> AnyView
    func failedView(_ error: Error) -> AnyView
    func loadedView(_ result: Result) -> AnyView
}
